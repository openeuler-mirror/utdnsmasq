 ...
 */
 ...
 use nix::sys::socket::{
     bind, setsockopt, socket, AddressFamily, InetAddr, SockAddr as SOCKADDR, SockFlag,
 ...
 pub fn enumerate_interfaces(
     mut interfacep: &mut Option<Box<Irec>>,
                     ...
                     || (ifr.ifr_ifru.ifru_flags & IFF_POINTOPOINT != 0))
             {
                 ...
                 while let Some(ref mut ctx) = context {
                             ...
                             == (ctx.end.s_addr & netmask.s_addr)
                     {
                         ...
                         setsockopt(fd, ReuseAddr, &opt)
                             .map_err(|e| format!("Failed to set SO_REUSEADDR: {}", e))?;
                         setsockopt(fd, Broadcast, &opt)
                             .map_err(|e| format!("Failed to set SO_BROADCAST: {}", e))?;
                         let saddr = SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::UNSPECIFIED, 67));
                         let sockaddr = SOCKADDR::new_inet(InetAddr::from_std(&saddr));
                         bind(fd, &sockaddr).map_err(|e| format!("Failed to bind socket: {}", e))?;
                     }
                 }
             }
         }
